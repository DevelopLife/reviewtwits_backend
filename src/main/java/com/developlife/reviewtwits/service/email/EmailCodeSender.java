...
public class EmailCodeSender extends EmailSender{
    ...
    @Autowired
    public EmailCodeSender(JavaMailSender emailSender, EmailVerifyRepository emailVerifyRepository, SpringTemplateEngine templateEngine) {
        super(emailSender, emailVerifyRepository, templateEngine);
    }

    protected MimeMessage createMessage(String email, String code) throws MessagingException, UnsupportedEncodingException {
        MimeMessage message = emailSender.createMimeMessage();

        message.addRecipients(MimeMessage.RecipientType.TO, email);//보내는 대상
        message.setSubject("ReviewTwits 회원가입 인증 메일입니다");//제목

        message.setText(setContext("registerMail", code), "utf-8", "html");//내용
